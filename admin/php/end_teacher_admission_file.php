...
if ($stmt) {
    $stmt->bind_param("s", $teacherId); // Bind learner_id to idno
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $email = $row['email'];
            $teacherNames = $row['firstname'] . ' ' . $row['secondname'];
            ...
            try {
                ...
                $mail->addAddress($email); // Add parent's email as recipient
