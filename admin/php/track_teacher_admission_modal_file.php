...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($status === null) {
        header("Location: ../php/track_teacher_admission.php");
        exit();
    }
    ...
    switch ($status) {
        case 'Pending':
            $query = "UPDATE teacher_admission SET status = 'Pending' WHERE id = ?";
            break;
        case 'Approved':
            $query = "UPDATE teacher_admission SET status = 'Approved' WHERE id = ?";
            break;
        case 'Rejected':
            $query = "UPDATE teacher_admission SET status = 'Rejected' WHERE id = ?";
            break;
        default:
            echo "<script>alert('Invalid status value!'); window.location.href='../php/track_teacher_admission.php';</script>";
            exit;
    }
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Status updated successfully!'); window.location.href='../php/track_teacher_admission.php';</script>";

        $sqlCheck = "SELECT * 
             FROM teacher_admission 
             WHERE id = ?";

        $stmtCheck = $conn->prepare($sqlCheck);
        $stmtCheck->bind_param("i", $id); // "i" = integer
        $stmtCheck->execute();
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            $status = strtolower($rowCheck['status']);;
            $surname = $rowCheck['surname'];
            $name = $rowCheck['firstname'];
            
            if($status==='approved'){
                $subjectHeader = 'Your MYCareerTrendz Application Has Been Approved';
                $message = '<p>Dear <b>' . $surname . ', ' . $name . '</b></p>
                ...
                $message = 'Dear <b>' . $surname . ', ' . $name . ',</b></p>
            ...
            try {
                ...
                $mail->addAddress($email); // Add learner's email as recipient
                // Content
        ...
        echo "<script>alert('Failed to update status!'); window.location.href='../php/track_teacher_admission.php';</script>";
        exit();
    }
