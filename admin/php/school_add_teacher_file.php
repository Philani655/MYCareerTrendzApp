...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt) {
        ...
        if ($result->num_rows > 0) {
            echo "<script>alert('Teacher already has a school!');window.location.href='../php/school_add_learner.php';</script>";
            exit();
        } else {
            ...
            if ($stmt) {
                ...
                // Execute the query
                if ($stmt->execute()) {
                    echo "<script>alert('School is inserted successfully!'); window.location.href='../php/school_teacher_page.php';</script>";
                    exit();
                } else {
                    echo "<script>alert('Something went wrong with adding school');window.location.href='../php/school_teacher_page.php';</script>";
                    exit();
                }

                // Close the statement
            } else {
                echo "<script>alert('Invalid query statements!');window.location.href='../php/school_teacher_page.php';</script>";
                exit();
            }
        }
    }
