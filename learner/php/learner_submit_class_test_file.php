...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { 
    ...
    $class_test_name = $_SESSION['class_test_name'];
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['document']['tmp_name'], $targetFilePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO `learner_class_test` 
    (`learner_id`, `test_id`, `subject_id`, `grade_id`, `file_name`, `file_path`, `class_test_name`, `description`, `score`) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ");
            $stmt->bind_param(
    "siiissssi", 
    $idNumber, $test_id, $subject_id, $grade_id, $fileName, $targetFilePath, $class_test_name, $description, $score);

            if ($stmt->execute()) {
                echo "<script>alert('File is uploaded to learners successfully!');</script>";
                echo "<script>window.location.href='../php/learner_class_test.php';</script>";
                exit();
            } else {
