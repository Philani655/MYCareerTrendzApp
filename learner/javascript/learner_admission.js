$(document).ready(function () {
    $('#learnerAdmission').submit(function (event) {
        ...
        if (isLearnerAdmission()) {
            alert("Step 1 completed successfuly!");
...
function isLearnerAdmission() {
    ...
    if (!validCertificate) {
        $('#validCertificate-error').text('Citizen is required').show();
        isValid = false;
    }

    // Validate ID Number using Luhn Algorithm
    if (!isValidLuhn(idNumber)) {
        $('#idNumber-error').text('Invalid ID Number. Please enter a valid ID Number.').show();
        isValid = false;
    }
