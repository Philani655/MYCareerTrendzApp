...
class Deprecation {
    ...
    const TYPE_NONE = 0;
    const TYPE_TRACK_DEPRECATIONS = 1;
    const TYPE_TRIGGER_ERROR = 2;
    const TYPE_PSR_LOGGER = 4;
    ...
    private static function delegateTriggerToBackend(string $message, array $backtrace, string $link, string $package): void {
        ...
        $message .= sprintf(
                ' (%s:%d called by %s:%d, %s, package %s)', self::basename($backtrace[0]['file'] ?? 'native code'), $backtrace[0]['line'] ?? 0, self::basename($backtrace[1]['file'] ?? 'native code'), $backtrace[1]['line'] ?? 0, $link, $package
        );
