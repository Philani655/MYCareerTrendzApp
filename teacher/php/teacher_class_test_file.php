...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $classTestName= $_POST['classTestName'];
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['document']['tmp_name'], $targetFilePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO teacher_class_test (teacher_id, subject_id, grade_id, file_name, file_path,class_test_name , description) 
                                    VALUES (?, ?, ?, ?, ?, ? ,?) ");
            $stmt->bind_param("sssssss", $idNumber, $subject_id, $grade_id, $fileName, $targetFilePath, $classTestName, $description);

            if ($stmt->execute()) {
                echo "<script>alert('Class test is uploaded successfully!');</script>";
                echo "<script>window.location.href='../php/teacher_class_test.php';</script>";
                exit();
            } else {
