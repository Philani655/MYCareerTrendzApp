...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO lesson_content (teacher_id, topic_id ,  subject_id, grade_id, content, date_time) 
            VALUES (?, ? , ?, ?, ?, ?)";

    // Use prepared statement
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("siiiss", $idNumber, $topic_id, $subject_id, $grade_id, $content, $date_time);
    ...
    if ($resultLearner->num_rows > 0) {
        while ($rowLearner = $resultLearner->fetch_assoc()) {
            ...
            $status = 0;
            $message = $content . ' ' . $_SESSION['subjectname'] . ' lesson';
    ...
    if ($stmt->execute() && $isValid) {
        echo "<script>alert('Teacher content is sent to learners');</script>";
        echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
        exit();
    } else {
        echo "<script>alert('Something went wrong with your subject!');</script>";
