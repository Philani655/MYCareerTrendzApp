...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmtCheck) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<script>window.location.href = '../php/teacher_register.php'</script>";
            exit();
        } else {
            ...
            $sqlLearner = "SELECT * FROM teacher WHERE idno = ?";
            ...
            if ($resultLearner->num_rows > 0) {
                echo "<script>alert('Teacher is already registered!');</script>";
                ...
                // Prepare the SQL query
                $sql = "INSERT INTO `teacher` (`idno`, `name`, `surname`, `email`, `password`, `contactno`, `location`, `grade_id`, `school_id`) 
