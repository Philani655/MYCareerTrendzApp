...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['document']['tmp_name'], $targetFilePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO reports (teacher_id, subject_id, grade_id, file_name, file_path,term , description) 
                                    VALUES (?, ?, ?, ?, ?, ? ,?) ");
            $stmt->bind_param("siissss", $idNumber, $subject_id, $grade_id, $fileName, $targetFilePath, $term, $description);

            if ($stmt->execute()) {
                echo "<script>alert('Report is uploaded successfully!');</script>";
                echo "<script>window.location.href='../php/teacher_quarterly_report.php';</script>";
                exit();
            } else {
