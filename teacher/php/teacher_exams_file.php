...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "INSERT INTO exam_scope (teacher_id, subject_id, grade_id, exam, date_time) 
            VALUES (?, ?, ?, ?, ?)";
    ...
    if ($resultLearner->num_rows > 0) {
        while ($rowLearner = $resultLearner->fetch_assoc()) {
            ...
            $status = 0;
            $message = $content . ' ' . $_SESSION['subjectname'] . ' exam scope';
            ...
            if ($stmttn->execute() && $stmtpn->execute()) {
                ...
                echo "<script>window.location.href='../php/teacher_exams.php';</script>";
                exit();
            }
        }
    }
    ...
    if ($stmt->execute() && $isValid) {
        echo "<script>alert('Exam scope is sent to learners');</script>";
        echo "<script>window.location.href='../php/teacher_exams.php';</script>";
        exit();
    } else {
        echo "<script>alert('Something went wrong with your subject!');</script>";
