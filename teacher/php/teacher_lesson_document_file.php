...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['document']['tmp_name'], $targetFilePath)) {
            ...
            $stmt = $conn->prepare("INSERT INTO lesson_documents (teacher_id, topic_id ,lesson_content_id ,  subject_id, grade_id, file_name, file_path, description) 
                                    VALUES (?, ?, ? ,?, ?, ?, ?, ?)");
            ...
            if ($resultLearner->num_rows > 0) {
                while ($rowLearner = $resultLearner->fetch_assoc()) {
                    ...
                    $status = 0;
                    $message = $fileName . ' ' . $_SESSION['subjectname'] . ' note lesson';
