<?php

session_start();
include_once '../database/config.php';

if ($_SERVER['REQUEST_METHOD'] == 'POST') {

    if (!isset($_SESSION['idNumber'])) {
        echo "<script>alert('Something went wrong');</script>";
        echo "<script>window.location.href='../php/teacher_dashboard.php'</script>";
        exit();
    }

    // Define the upload directory
    $uploadDir = '../../lessons/';

    // Check if the directory exists, if not, create it with proper permissions
    if (!is_dir($uploadDir)) {
        mkdir($uploadDir, 0777, true); // Create directory with full permissions
        chmod($uploadDir, 0777);       // Ensure directory is writable
    }

    // Get session data
    $idNumber = $_SESSION['idNumber'];
    $grade_id = $_SESSION['grade_id'];
    $subject_id = $_SESSION['subject_id'];
    $description = $_POST['description'];
    $topic_id = $_POST['topic_id'];
    $lesson_content_id = $_POST['lesson_content_id'];
    $subjectname = $_SESSION['subjectname'];
    $subjectcode = $_SESSION['subjectcode'];

    // Check if a video is uploaded and no error
    if (isset($_FILES['video']) && $_FILES['video']['error'] === 0) {

        // Get file details
        $fileName = basename($_FILES['video']['name']);
        $fileType = mime_content_type($_FILES['video']['tmp_name']);
        $fileSize = $_FILES['video']['size'];
        $targetFilePath = $uploadDir . time() . "_" . $fileName;

        // Allowed video file types (mp4, avi, mov, mkv)
        $allowedTypes = ['video/mp4', 'video/x-msvideo', 'video/quicktime', 'video/x-matroska'];
        if (!in_array($fileType, $allowedTypes)) {
            die("Invalid file type. Only MP4, AVI, MOV, and MKV are allowed.");
        }

        // Check file size (Optional: limit to 50MB)
        if ($fileSize > 50 * 1024 * 1024) {
            die("File size exceeds the limit of 50MB.");
        }

        // Move the uploaded file to the lesson_videos folder
        if (move_uploaded_file($_FILES['video']['tmp_name'], $targetFilePath)) {
            // Save file information into the database
            $stmt = $conn->prepare("INSERT INTO lesson_video (teacher_id, topic_id , lesson_content_id ,  subject_id, grade_id, video_name, video_path, description) 
                                    VALUES (?, ?, ?, ? ,?, ?, ?, ?)");
            $stmt->bind_param("siiiisss", $idNumber, $topic_id, $lesson_content_id, $subject_id, $grade_id, $fileName, $targetFilePath, $description);

            // Prepare the statement
            $sqllearner = "SELECT l.idno AS idno 
                            FROM learner l, subjects su 
                            WHERE l.idno = su.learner_id 
                            AND l.grade_id = su.grade_id 
                            AND l.grade_id = ? 
                            AND su.subjectcode = ? 
                            AND su.subjectname = ?";

            $stmtLearner = $conn->prepare($sqllearner);
            $stmtLearner->bind_param("iss", $grade_id, $subjectcode, $subjectname);
            // "i" = integer, "s" = string
            // Execute the statement
            $stmtLearner->execute();
            $resultLearner = $stmtLearner->get_result();

            $isValid = false;

            if ($resultLearner->num_rows > 0) {
                while ($rowLearner = $resultLearner->fetch_assoc()) {

                    $learner_id = $rowLearner['idno'];
                    $status = 0;
                    $message = $fileName . ' ' . $_SESSION['subjectname'] . ' video lesson';

                    // Prepare the statement
                    $sqltn = "INSERT INTO `teacher_notifications`(`teacher_id`, `learner_id`, `subject_id`, `grade_id`, `message`, `status`) VALUES (?,?,?,?,?,?)";
                    $stmttn = $conn->prepare($sqltn);
                    $stmttn->bind_param("ssiisi", $idNumber, $learner_id, $subject_id, $grade_id, $message, $status);
                    // "iiiis" means: integer, integer, integer, integer, string
                    // Prepare the statement
                    $sqlpn = "INSERT INTO `parent_notifications`(`teacher_id`,`learner_id` , `subject_id`,  `grade_id`, `message`, `status`) VALUES (?,? ,?,?,?,?)";
                    $stmtpn = $conn->prepare($sqlpn);
                    $stmtpn->bind_param("ssiisi", $idNumber, $learner_id, $subject_id, $grade_id, $message, $status);
                    // "iiiis" means: integer, integer, integer, integer, string

                    if ($stmttn->execute() && $stmtpn->execute()) {
                        $isValid = true;
                    } else {
                        echo "<script>alert('Something went wrong with your subject!');</script>";
                        echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
                        exit();
                    }
                }
            }

            if ($stmt->execute() && $isValid) {
                echo "<script>alert('Video is uploaded to learners successfully!');</script>";
                echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
                exit();
            } else {
                echo "<script>alert('Failed to save video!');</script>";
                echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
                exit();
            }

            $stmt->close();
        } else {
            echo "<script>alert('Error moving the video.!');</script>";
            echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
            exit();
        }
    } else {
        echo "Error: " . $_FILES['video']['error'];
    }
} else {
    echo "<script>alert('Something went wrong with your subject!');</script>";
    echo "<script>window.location.href='../php/teacher_lessons_page.php';</script>";
    exit();
}
?>
