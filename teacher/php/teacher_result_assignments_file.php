...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        // Prepare the statement
            ...
            if ($resultLearner->num_rows > 0) {
                while ($rowLearner = $resultLearner->fetch_assoc()) {
                    ...
                    $status = 0;
                    $message = 'Assignment final mark is updated in ' . $_SESSION['subjectname'] ;
        ...
        $sqlInsert = "INSERT INTO subject_final_mark 
            (teacher_id, learner_id, term_id,subject_id , grade_id , assignment_mark) 
        VALUES (?, ?, ?, ? , ? , ?)";
        ...
        // Prepare the statement
            ...
            if ($resultLearner->num_rows > 0) {
                while ($rowLearner = $resultLearner->fetch_assoc()) {
                    ...
                    $learner_id = $rowLearner['idno'];
                    $status = 0;
                    $message = 'Assignment final mark in ' . $_SESSION['subjectname'] ;
