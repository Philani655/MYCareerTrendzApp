    ...
    if ($result->num_rows > 0) {
        ...
        if ($resultLearner->num_rows > 0) {
            while ($rowLearner = $resultLearner->fetch_assoc()) {
                ...
                $status = 0;
                $message = 'Final mark is updated in ' . $_SESSION['subjectname'];
        ...
        if ($stmtUpdate->affected_rows > 0 && $isValid) {
            ...
            echo "<script>alert('Fail to updated marks');window.location.href='../php/.php';</script>";
            exit();
        }
    } else {
        $sqlInsert = "INSERT INTO subject_final_mark 
            (teacher_id, learner_id, term_id, subject_id ,  grade_id , final_mark) 
        VALUES (?, ?, ?, ? , ? , ?)";
        ...
        if ($resultLearner->num_rows > 0) {
            while ($rowLearner = $resultLearner->fetch_assoc()) {
                ...
                $learner_id = $rowLearner['idno'];
                $status = 0;
                $message = 'Final mark in ' . $_SESSION['subjectname'];
